...
let GalleryDeluxe = {
	init: async function () {
		...
		var options = {
			...
			onHoverHandler: function (filename) {
				debug('onHoverHandler', filename);
				activeImage = imagesMap.get(filename);
				if (activeImage) {
					let date = new Date(activeImage.exif.Date)
					var dataData = new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().split('T')[0].split('-');
					document.getElementById("disp-year").innerHTML = dataData[0]
					document.getElementById("disp-month").innerHTML = dataData[1]
					document.getElementById("disp-day").innerHTML = dataData[2]
				}
			},
